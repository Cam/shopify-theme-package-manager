...
export async function buildPackages() {
  ...
  if (pkgNames.length === 0) {
    console.log(chalk.yellow(`[stpm] No packages found in manifest.`));
    return;
  }
  ...
  for (const pkgName of pkgNames) {
    ...
    if (entry.native) {
      console.log(chalk.cyan(`[stpm] Rebuilding ${pkgName} as STPM-native...`));
      await importNativePackage(pkgName);
      continue;
    }

    if (entry.bundled) {
      console.log(chalk.cyan(`[stpm] Rebuilding ${pkgName} as bundled...`));
      for (const item of entry.bundled) {
        ...
        if (item.type === 'css') {
          ...
          console.log(chalk.green(`[stpm] Rebuilt CSS → ${item.output}`));
        }
        ...
        if (item.type === 'js') {
          ...
          await normalizeFileSpacing(outputPath);
          console.log(chalk.green(`[stpm] Rebuilt JS → ${item.output}`));
        }
      }
    }
  }

  console.log(chalk.green(`[stpm] Build complete.`));
}